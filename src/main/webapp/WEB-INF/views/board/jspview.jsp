...
<body class="page-on-scroll ">
                        ...
                        <div class="my_box" data-height="height">
                            <form method="post">
                                <div class="col-md-6">
									...
									<input type="hidden" name="post_id" id = "post_id" value="${jspview.post_id }">
	                            ...
	                            <div class="card-body">
									...
									<div class="d-flex col-md-12 p-0" style="margin-bottom : 40px;">
		                                <form id="replyForm" name="replyForm" method="post" class="mb-4 d-flex col-md-12 p-0" target=detail onsubmit="all_reset();">
		                                	...
		                                	<input type="hidden" id="board_category_id" name="board_category_id" value="${jspview.board_category_id}">
	...
	function undo_process(){
		...
		var postValue = '<c:out value="${jspview.post_id }"/>'
		var replyUL = $(".chat");
		showList(1);
	...
	function showList(page) {
			var postValue = '<c:out value="${jspview.post_id }"/>'
			var board_category_idValue = '<c:out value="${jspview.board_category_id}"/>'
			...
			replyService.getList({ board_category_id : board_category_idValue, post_id : postValue, page : page || 1},function(list) {
				...
				for (var i = 0, len = list.length || 0; i < len; i++) {
					...
					str += "<c:if test='${member.user_nickname eq jspview.user_nickname}'>";
		...
		function updateviewBtn(reply_id, reply_regdate, reply_contents,user_nickname) {
			var updatestr = "";
				...
				updatestr += "<c:if test='${member.user_nickname eq jspview.user_nickname}'>";
