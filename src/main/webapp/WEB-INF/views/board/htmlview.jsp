...
<body class="page-on-scroll ">
                        ...
                        <div class="my_box" data-height="height">
                            <form method="post">
                                <div class="col-md-6">
									...
									<input type="hidden" name="post_id" id = "post_id" value="${htmlview.post_id }">
                                ...
                                <div>
                                    <pre class="form-control" placeholder="내용을 입력해 주세요." style="height : 650px; resize: none; background-color: #fff; border:none;" disabled>${htmlview.post_contents }</pre>
                                </div> 
                                <div style="text-align:center; border-bottom : 1px solid #d9d9d9;">
									...
									<p id="post_rec" style="color: #000;">${htmlview.post_rec}</p>
								</div>                
	                        	<div class="mb-5">
	                            ...
	                            <div class="card-body">
									...
									<div class="d-flex col-md-12 p-0" style="margin-bottom : 40px;">
		                                ...
		                                <form id="replyForm" name="replyForm" method="post" class="mb-4 d-flex col-md-12 p-0" target=detail onsubmit="all_reset();">
		                                	...
		                                	<input type="hidden" id="board_category_id" name="board_category_id" value="${htmlview.board_category_id}">
	...
	<div class="modal fade" id="memberreport" tabindex="-1" role="dialog" aria-labelledby="memberreport" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<form name="Check" action="/board/htmlreport" id="reportForm" method ="get" >
					<input type="text" style="display:none;" name="reportee_index" id="reportee_index" value="${htmlview.user_index }">
					<input type="text" style="display:none;" name="reporter_index" id="reporter_index" value="${member.user_index }">
					<input type="text" style="display:none;" name="board_category_id" id="board_category_id" value="${htmlview.board_category_id }">
					<input type="text" style="display:none;" name="post_id" id="post_id" value="${htmlview.post_id }">
	...
	function undo_process(){
		...
		var postValue = '<c:out value="${htmlview.post_id }"/>'
		var replyUL = $(".chat");
		showList(1);
	...
	function showList(page) {
			var postValue = '<c:out value="${htmlview.post_id }"/>'
			var board_category_id = '<c:out value="${htmlview.board_category_id}"/>'
			...
			replyService.getList({ board_category_id : board_category_id, post_id : postValue, page : page || 1},function(list) {
				...
				for (var i = 0, len = list.length || 0; i < len; i++) {
					...
					str += "<c:if test='${member.user_nickname eq htmlview.user_nickname}'>";
		...
		function updateviewBtn(board_category_id, reply_id, reply_regdate, reply_contents,user_nickname) {
			var updatestr = "";
				...
				updatestr += "<c:if test='${member.user_nickname eq htmlview.user_nickname}'>";
