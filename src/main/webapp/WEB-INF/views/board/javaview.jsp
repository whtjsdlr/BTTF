...
<body class="page-on-scroll ">
                        ...
                        <div class="my_box" data-height="height">
                            <form method="post">
                                <div class="col-md-6">
									...
									<input type="hidden" name="post_id" id = "post_id" value="${javaview.post_id }">
                                    <p style="color: black; font-size: 2rem; font-weight:bold;">글 제목 : ${javaview.post_subject }</p>
                                </div>
                                <div class="col-md-2">
                                	<p class="margin-b-50 text-center" >조회수 ${javaview.post_vcount }</p>
                                </div>
                                <div class="col-md-2">
                                	<p class="margin-b-50 text-center" > 작성자 ${javaview.user_nickname }</p>
                                </div>
                                <div class="col-md-2">
                                	<p class="margin-b-50 text-center" > <fmt:formatDate value="${javaview.post_regdate}" pattern="yyyy-MM-dd HH:mm" /></p>
                                </div>
                                <div>
                                    <pre class="form-control" placeholder="내용을 입력해 주세요." style="height : 650px; resize: none; background-color: #fff; border:none;" disabled>${javaview.post_contents }</pre>
                                </div> 
                           	    <div style="text-align:center; border-bottom : 1px solid #d9d9d9;">
									...
									<p id="post_rec" style="color: #000;">${javaview.post_rec}</p>
								</div> 
	                        	<div class="mb-5">
									<c:if test="${member.user_nickname eq javaview.user_nickname}">
		 		                    	<a href="/board/javamodify?post_id=${javaview.post_id }" class="btn btn-primary mt-4" id="list" type="submit">글수정</a>                          
				                		<a href="/board/javadelete?post_id=${javaview.post_id }&mypage=" class="btn btn-danger mt-4" id="list" type="submit">글삭제</a>
									</c:if> 
	                        		<a href="/board/javalist" class="btn btn-default mt-4" id="edit" type="submit">글 목록</a>
									<c:if test="${member.user_nickname != javaview.user_nickname && member != null && javaview.user_nickname != 'admin'}">
	                            ...
	                            <div class="card-body">
									...
									<div class="d-flex col-md-12 p-0" style="margin-bottom : 40px;">
		                                ...
		                                <form id="replyForm" name="replyForm" method="post" class="mb-4 d-flex col-md-12 p-0" target=detail onsubmit="all_reset();">
		                                	...
		                                	<input type="hidden" id="post_id" name="post_id" value="${javaview.post_id}">
	...
	<div class="modal fade" id="memberreport" tabindex="-1" role="dialog" aria-labelledby="memberreport" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<form name="Check" action="/board/javareport" id="reportForm" method ="get" >
					<input type="text" style="display:none;" name="reportee_index" id="reportee_index" value="${javaview.user_index }">
					<input type="text" style="display:none;" name="reporter_index" id="reporter_index" value="${member.user_index }">
					<input type="text" style="display:none;" name="board_category_id" id="board_category_id" value="${javaview.board_category_id }">
					<input type="text" style="display:none;" name="post_id" id="post_id" value="${javaview.post_id }">
	...
	var input = document.getElementById("reply_contents");
	   input.addEventListener("keypress", function(event) {
		...
		var postValue = '<c:out value="${javaview.post_id }"/>'
		var replyUL = $(".chat");
		showList(1);
	...
	function showList(page) {
			var postValue = '<c:out value="${javaview.post_id }"/>'
			var board_category_id = '<c:out value="${javaview.board_category_id}"/>'
			...
			replyService.getList({ board_category_id : board_category_id, post_id : postValue, page : page || 1},function(list) {
				...
				for (var i = 0, len = list.length || 0; i < len; i++) {
					...
					str += "<c:if test='${member.user_nickname eq javaview.user_nickname}'>";
		...
		function updateviewBtn(board_category_id, reply_id, reply_regdate, reply_contents,user_nickname) {
			var updatestr = "";
				...
				updatestr += "<c:if test='${member.user_nickname eq javaview.user_nickname}'>";
