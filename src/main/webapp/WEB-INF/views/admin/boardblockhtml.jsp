                   					...
                   					<c:when test = "${boardblockhtml != null and fn:length(boardblockhtml) > 0 }">
                   						<c:forEach var="html" items="${boardblockhtml}">
											<tr>
												...
												<td><fmt:formatDate pattern="yyyy-MM-dd" value="${html.post_regdate }"/></td>
												<td>
													 <a href="/board/htmldelete?post_id=${html.post_id}&mypage=admin" class="btn btn-danger">게시물 삭제</a>
												</td>
											</tr>
										</c:forEach>
									</c:when>
