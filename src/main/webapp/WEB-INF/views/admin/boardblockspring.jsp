                   					...
                   					<c:when test = "${boardblockspring != null and fn:length(boardblockspring) > 0 }">
                   						<c:forEach var="spring" items="${boardblockspring}">
											<tr>
												...
												<td><fmt:formatDate pattern="yyyy-MM-dd" value="${spring.post_regdate }"/></td>
												<td>
													 <a href="/board/springdelete?post_id=${spring.post_id}&mypage=admin" class="btn btn-danger">게시물 삭제</a>
												</td>
											</tr>
										</c:forEach>
									</c:when>
